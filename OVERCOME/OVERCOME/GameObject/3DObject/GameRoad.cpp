...
void GameRoad::Render(MatrixManager* matrixManager)
{
	...
	for (int j = 0; j < m_maxFloorBlock; j++)
	{
		for (int i = 0; i < m_maxFloorBlock; i++)
		{
			...
			{
			case 0: break;                                                                                                                  // 何もなし
			case 1: m_modelRoadStraight->Draw(res.GetD3DDeviceContext(), *CommonStateManager::SingletonGetInstance().GetStates(), world, matrixManager->GetView(), matrixManager->GetProjection());  break;   // 直線道路
			case 2: m_modelRoadStop->Draw(res.GetD3DDeviceContext(), *CommonStateManager::SingletonGetInstance().GetStates(), world, matrixManager->GetView(), matrixManager->GetProjection());      break;   // 末端道路
			case 3: m_modelRoadCurve->Draw(res.GetD3DDeviceContext(), *CommonStateManager::SingletonGetInstance().GetStates(), world, matrixManager->GetView(), matrixManager->GetProjection());     break;   // 曲線道路
			case 4: m_modelRoadBranch->Draw(res.GetD3DDeviceContext(), *CommonStateManager::SingletonGetInstance().GetStates(), world, matrixManager->GetView(), matrixManager->GetProjection());    break;   // 分岐道路
			}
			// デバッグ道路描画
